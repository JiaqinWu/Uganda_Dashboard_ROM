...
def app():
    # Main page content
    st.set_page_config(page_title = 'ROM Dashboard -- Uganda SCORE Survey', page_icon='🇺🇬',layout='wide')

    title = 'Comparison of Scores of Questions Within One Section Within ROM'
    ...
    available_scores = df[(df['Program'] == 'ROM') & (df['Module'] == module_selected) & (df['Part'] == part_selected)]['Score'].unique()
    ...
    if not filtered_data.empty:
        ...
        if plot_selected == 'Bar Plot':
            ...
            chart = alt.Chart(filtered_data).mark_bar().encode(
                ...
                width=600,
                height=600,
                title=f'ROM -- Bar Plot of Scores by Question within {module_selected1}: {part_selected}'
            )
            ...
            final_chart1 = alt.layer(pie, text1).properties(
                width=600,
                height=400,
                title=f'ROM -- Pie Plot of Scores by Question within {module_selected1}: {part_selected}'
            ).configure_axis(
                labelFontSize=12,
                titleFontSize=14
            ).interactive()
            ...
            fig = px.line_polar(filtered_data, r='Score', theta='Question1', line_close=True,
                                ...
                                title=f'ROM -- Radar Plot of Scores by Question within {module_selected1}: {part_selected}',
                                hover_data={
                                    'Question': True,
                                    'Score': True,  
                                    'Level': True,  
            ...
            st.markdown(f"#### Comparison of Score by Questions in ROM  are shown below:")
